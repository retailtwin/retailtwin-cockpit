-- Drop the incorrectly placed function from aifo schema
DROP FUNCTION IF EXISTS aifo.merge_sales_inventory_to_fact_daily(uuid);

-- Create function in public schema (where Supabase RPC expects it)
CREATE OR REPLACE FUNCTION public.merge_sales_inventory_to_fact_daily(p_dataset_id uuid)
RETURNS TABLE(
