-- Update get_kpi_data_aggregated to use value-based calculations with cost/price data
...
BEGIN
  -- Calculate total sales value and average inventory value (joining with products for costs)
  SELECT 
    COALESCE(SUM(fd.units_sold * p.unit_price), 0),
    COALESCE(AVG(fd.on_hand_units * p.unit_cost), 0),
    COALESCE(AVG(fd.on_hand_units_sim * p.unit_cost), 0),
    COALESCE(SUM(fd.units_sold), 0)
  INTO v_total_sales_value, v_avg_inventory_value, v_avg_inventory_value_sim, v_missed_units
  ...
  SELECT 
    COALESCE((MAX(d) - MIN(d))::integer + 1, 0)
  ...
  -- Calculate inventory turns: total sales value / average inventory value
