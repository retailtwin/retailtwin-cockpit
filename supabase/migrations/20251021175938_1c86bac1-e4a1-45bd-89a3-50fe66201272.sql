-- Phase 1: Clean duplicate data from fact_daily table
-- Delete duplicates, keeping the row with most complete data (non-null target_units and economic_units preferred)
WITH ranked_rows AS (
  SELECT 
    id,
    ROW_NUMBER() OVER (
      PARTITION BY location_code, sku, d 
      ORDER BY 
        CASE WHEN target_units IS NOT NULL THEN 1 ELSE 0 END DESC,
        CASE WHEN economic_units IS NOT NULL THEN 1 ELSE 0 END DESC,
        id DESC
    ) as rn
...
ADD CONSTRAINT unique_location_sku_date 
UNIQUE (location_code, sku, d);

-- Phase 2 & 3: Update get_kpi_data_aggregated function with corrected service level and inventory turns calculations
...
DECLARE
  ...
  v_riv numeric;
BEGIN
  -- Calculate total sales value at cost and total units sold
  SELECT 
  ...
  -- FIXED: Calculate average inventory value by summing daily totals, then averaging
  SELECT 
  ...
  -- FIXED: Calculate service level per SKU-location, then weighted average (always <= 100%)
  ...
  -- FIXED: Calculate simulated service level per SKU-location, then weighted average
  ...
  -- Calculate TCM (Total Contribution Margin)
  WITH margin_calc AS (
    SELECT 
  ...
  -- Calculate MTV (Marginal Throughput Value)
  v_mtv := CASE 
    WHEN v_service_level > 0 THEN (v_tcm / v_service_level) - v_tcm
    ELSE 0
  END;

  -- Calculate RIV using the function with date range
